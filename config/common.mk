...
ifeq ($(CM_BUILDTYPE), RELEASE)
    ...
    ifeq ($(CM_VERSION_MAINTENANCE),0)
        LINEAGE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-LegendROM-12.0$(CM_EXTRAVERSION)-$(CM_BUILD)
    else
        LINEAGE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(CM_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)-LegendROM-12.0$(CM_EXTRAVERSION)-$(CM_BUILD)
    endif
endif
